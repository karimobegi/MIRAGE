...
public class HINTRunner : InpaintingRunner, IEffectHandler
{   
...
#region Model Preparation
    protected override void PrepareModel()
    {
        ...
        // HINT expects 1 = missing, 0 = known — no inversion needed
    ...
    {
        if (objectValidator == null) return;
...
#region Model Execution
    ...
    protected override void ReadOutput()
    {
        ...
        var maskClone = inputMaskTensor.ReadbackAndClone();
        float maskSum = 0;
        for (int i = 0; i < maskClone.shape.length; i++) maskSum += maskClone[i];
        Debug.Log($"Mask sum: {maskSum} / {maskClone.shape.length} (ratio: {maskSum / maskClone.shape.length})");
        maskClone.Dispose();
